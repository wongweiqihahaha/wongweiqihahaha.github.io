...
```{r, out.height= "400px",out.width= "800px",echo=TRUE,eval=TRUE,fig.cap="There is a caption on how to read for help"}
knitr::include_graphics("readhelp.png")
```
...
**Solution:** *It allows you to skip rows wwith missing values*
...
ggplot(data=playlist_data) + aes(x=Age,y=Rating) +
geom_point() +
geom_point(shape = 5, colour = "black", fill = "white", size = 5, stroke = 5) # <-- Hint: Use ? to learn more about geom_point and use appropriate values for shape
...
ggplot(data=playlist_data) + aes(x=Age,y=Rating) +
geom_point() +
geom_point(shape = 1, colour = "black", fill = "white", size = 5, stroke = 5) +
labs(title="Age against Rating",caption="This tells you the Age and Rating of DJs!")
```
